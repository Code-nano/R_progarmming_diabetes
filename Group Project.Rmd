...
```{r}
# Define the model formula
classification_model <- Diabetes_012 ~ .

set.seed(123)  # Set a random seed for reproducibility for Classification method
train_index_classification <- createDataPartition(data$Diabetes_012, p = 0.6, list = FALSE)
train_data_classification <- data[train_index_classification, ]
temp_data_classsification <- data[-train_index_classification, ]

# Create validation and test sets
validation_index_classification <- createDataPartition(temp_data_classsification$Diabetes_012, p = 0.5, list = FALSE)
validation_data_classification <- temp_data_classsification[validation_index_classification, ]
test_data_classification <- temp_data_classsification[-validation_index_classification, ]


#naive bayes model
nb_model <- NaiveBayes(as.factor(data$Diabetes_012),grouping=as.factor(data$Diabetes_012), data=train_data_classification, userkernel = FALSE, fL = 0,)
plot(nb_model)
#make predictions
nb_x_test <- test_data_classification[,2:20]
nb_y_test <- test_data_classification[,1]
predictions_nb <- predict(nb_model)
##confusionMatrix(predictions_nb, nb_y_test)
```

```{r}
knn_model <- sknn(as.factor(data$Diabetes_012),grouping=as.factor(data$Diabetes_012), data=train_data_classification, method="knn")
#make predictions
knn_x_test <- test_data_classification[,2:22]
knn_y_test <- test_data_classification[,1]
...
library(randomForest)
rf_model <- randomForest(Diabetes_012 ~ ., data = train_data_classification)
rf_x_test <- test_data_classification[,2:22]
rf_y_test <- test_data_classification[,1]
