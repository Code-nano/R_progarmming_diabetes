...
# Now, rebuild the model with the best mtry value using the train function
set.seed(123)
final_model_rf <- train(Diabetes_012 ~ ., data = trainData, method = "rf", trControl = trainControl(method = "cv", number = 10), tuneGrid = data.frame(.mtry=best_mtry))
...
final_predictions <- predict(final_model_rf, testData)

# Generate predicted probabilities for ROC analysis
final_prob_predictions <- predict(final_model_rf, testData, type = "prob")

# Ensure final_predictions are factors with the same levels as the actual data
final_predictions <- factor(final_predictions, levels = levels(testData$Diabetes_012))

# Confusion Matrix
final_confMat <- confusionMatrix(final_predictions, testData$Diabetes_012)
print(final_confMat)

# Overall Accuracy
final_accuracy <- sum(final_predictions == testData$Diabetes_012) / length(final_predictions)
print(paste("Accuracy:", final_accuracy))
...
calculate_roc_auc <- function(class, predictions, actual_labels) {
  ...
  binary_response <- actual_labels == class
  # Extract predicted probabilities for the current class
  predicted_probabilities <- predictions[, as.character(class)]
...
final_roc_results <- lapply(levels(testData$Diabetes_012), calculate_roc_auc, final_prob_predictions, testData$Diabetes_012)

# Printing AUC values for each class
sapply(final_roc_results, function(x) x$auc)
...
calculate_metrics <- function(class, predictions, actual_labels) {
  # Convert to binary format (one-vs-rest approach)
  binary_predictions <- predictions == class
  binary_actual_labels <- actual_labels == class
  # Calculate metrics
  precision <- posPredValue(binary_predictions, binary_actual_labels)
  recall <- sensitivity(binary_predictions, binary_actual_labels)
  f1_score <- ifelse((precision + recall) > 0, (2 * precision * recall) / (precision + recall), 0)
  c(Precision = precision, Recall = recall, F1_Score = f1_score)
}

# Calculating metrics for each class
final_class_metrics <- sapply(levels(testData$Diabetes_012), function(class) calculate_metrics(class, final_predictions, testData$Diabetes_012))

# Print the metrics for each class
print(final_class_metrics)# Generate class predictions
final_predictions <- predict(final_model_rf, testData)
